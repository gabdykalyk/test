const svg=new Vivus("back",{type:"delayed",duration:400});let formEmail=document.querySelector("#formEmail"),message=document.querySelector(".error-msg"),password=document.querySelector("#formPassword");formEmail.addEventListener("input",()=>{formEmail.validity.valid||formEmail.classList.add("error"),formEmail.validity.valid&&formEmail.classList.remove("error")}),password.addEventListener("input",()=>{password.validity.valid||(password.classList.add("error"),message.style.opacity="1"),password.validity.valid&&(password.classList.remove("error"),message.style.opacity="0")}),
/**
